...
export async function POST(request: Request) {
  try {
    ...
    const sheetExists = sheetInfo.data.sheets?.some(
      (sheet) => sheet.properties?.title === "Registration"
    );
    ...
    if (rows.length === 0) {
      ...
      await sheets.spreadsheets.values.append({
        ...
        requestBody: {
          values: [["S.N.", "Date", "Full Name", "Email", "Phone", "Country"]],
        },
      });
    }
    ...
    await sheets.spreadsheets.values.append({
      ...
      valueInputOption: "RAW",
      requestBody: {
        values: [[serialNumber, date, fullName, email, phone, country]],
      },
    });
    ...
    const mailOptions = {
      ...
      text: `
        You have received a new Event Registration:
